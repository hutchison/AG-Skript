...
In modernen Texten werden 1. und 2. oft zusammengefasst:
...
\centerline{\includegraphics[width=8cm]{BILDER/BildKonstruktionGleichseitigesDreieck.pdf}}
...
kompliziert und wurde von Gauß (im Alter von 26) % bereits als 19
                                % jähriger den Primzahlfall gelöst!
...
Skalierung einer Geraden. % durch wiederholte Addition. 
% Man beachte, dass Längen immer nur im Verhältnis zu Punkten gegeben sind
% (Propotionslehre bla bla)
...
Konstruktions-Axiomen ist nicht möglich! % Referenzen??
...
Seite von $h$. % irgendeiner der Geraden

\item
Die inneren Wechselwinkel an $h$ sind gleich (kongruent).

\item
{\em Innenwinkelsatz}: Die Summe der Innenwinkel in einem Dreieck ist $\pi$. % (2Rechte).
...
% Dazu muss man in GeoGebra erst einen Punkt ausserhalb der Geraden
% durch $A$ und $B$ erzeugen!
...
\centerline{\includegraphics[width=14cm]{BILDER/BildKonstruktionNTeilungStrecke.pdf}}
...
\centerline{\includegraphics[width=8cm]{BILDER/BildStrahlensatz.pdf}}
...
\glqq Produkt von Strecken\grqq\ (notiert mit $AB\times CD$)  %Prüfen ob das stimmt!?
...
Aus dem Dreieck $OEA$ in obiger Konstruktion lässt sich auch 
...
mit $|A'B''|=|AB|$. 
% Existenz wird durch Schnitt von Kreis und Gerade "gewehrleistet"...
...
%Man beachte, dass wir hier eine "Superposition" (Bewegung) des
%Dreiecks $ABC$ vorgenommen haben, auf Basis der Kongruenz-Kriterien.
...
Eine Folgerung des Satzes ist die Feststellung, 
dass die Diagonale in einem Quadrat mit Kantenlänge~$1$ die 
irrationale Länge $\sqrt{2}$ %(Symbol für die positive Lösung der quadratischen Gleichung $x^2=2$)
...
Bleibt die Frage zu klären, wie wir zu einer gegeben Strecke der Länge
...
%Zweiteilung des Winkels behandelt...
\phantom{Pups}\hspace*{1cm}\phantom{Pups}\\
\underline{Gegeben:} Strecke~$AB$, mit Senkrechte~$g$ % zur Gerade durch
...
Der Beweis, dass diese Konstruktion funktioniert, basiert auf der
...
\centerline{\includegraphics[width=6cm]{BILDER/BildFlaecheDreieck.pdf}}
...
Strahlensatz. %\ref{thm:strahlensatz}
...
Der im Bild angedeutete Beweis für den Satz von Pythagoras zeigt in
...
{\em Kritik:} Der Beweis des Strahlensatzes verwendet das
...
%
% Anmerkung aus Nesselmann-Skript: 
