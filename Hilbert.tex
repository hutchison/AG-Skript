...
% BEM (taken from Wikipedia, http://en.wikipedia.org/wiki/Hilbert%27s_axioms):
%Other well-known modern axiomatizations of Euclidean geometry are
%those of Alfred Tarski and of George Birkhoff.



% ETWAS FORMALER:
% Formal "richtig" waere es eine Menge von Punkten $\P$ einzuführen
% und eine Menge von Geraden $\MG$ von Teilmengen von $\P$ (also
% \MG\subseteq 2^{\P}$. Eine Ebene~$\E$ ist dann ein Tripel $(\P,\MG,
...
{\bf Modell 4:} ( \glqq Bierdeckelgeometrie\grqq )\\ % nach Felix Klein
...
%% UEBUNG: (??)
%% Man kann zeigen, dass jede Gerade einer endlichen affinen
%% Inzidenzgeometrie die gleiche Anzahl von Punkten hat (wie?).
%% Diese Anzahl q wird ueblicherweise die Ordnung der Ebene genannt.
%% Man kann ferner zeigen, dass die Ebene der Ordnung q, q+1 Geraden
%% durch jeden Punkt, und insgesamt q^2 Punkte und q^2+q Geraden besitzt.
%
%% Affine Ebenen der Ordnung $n$ gibt es immer, wenn n eine
%% Primzahlpotenz ist.
%% Für $n=10$ wurde die Nichtexistenz mittels Computer-assisted-proof
%% bewiesen:
...
%% Historische Bemerkung zu einem Zitat von Gauß, der erkannt hatte,
%% dass man die Relation "Zwischen" vernünftig erklären muss?


%% INTERESSANTER HINTERGRUND:
% In Hilbert's Buch (1 Ausgabe von 1899 / siehe auch die Englische
% Übersetzung) gibt es noch ein weiteres Axiom der Anordnung, von dem
% aber "R. L. Moore proved that this axiom is redundant, in 1902".
% see http://en.wikipedia.org/wiki/Hilbert%27s_axioms
% In der 2. Auflage von 1903 ist dieses Axiom nicht mehr angeführt.
% In beiden Ausgaben ist aber die Rede davon, dass es einfach sei die
% Unabhängigkeit der Axiome voneinander zu beweisen!!!! :-) 
% (siehe Anfang von §10)
...
Oder: \glqq Ein Punkt %$A$ 
liegt {\em auf der anderen Seite} einer Geraden\grqq .
Konzepte wie \glqq {\em Innen}\grqq\  und \glqq {\em Außen}\grqq\
...
%BEM:   (Es ist ausreichend: \glqq{}h\"{o}chstens\grqq{} einen zu fordern!)
% Beweis?!?!?
...
%und das Prinzip \glqq{}Ebene\grqq{}   %% WAS IST DAMIT GEMEINT?!?
zu erkl\"{a}ren, 
ben\"{o}tigt man ein weiteres Axiom:

%%
%% ACHTUNG: Die Teilung der Ebene in zwei Hälften ist als Axiom
%% irgendwie einfacher (??), und das Axiom von Pasch ist dazu äquivalent. 
%% Beim nächsten Mal sollte man daher vielleicht ehr das "Theorem über
%% die Teilung der Ebene" siehe
%%
...
%AUS WIKIPEDIA:
%Anschaulich gesprochen: "Wenn eine Gerade durch eine Seite ins Innere
%eines Dreiecks eintritt, so tritt sie gewiss auch wieder durch eine
%Seite des Dreiecks heraus.
...
%In den folgenden S\"{a}tzen seien die Axiome der Inzidenz und der
%Anordnung f\"{u}r die betrachtete Ebene~$\E$ erf\"{u}llt.
...
%% ACHTUNG:
% Oft (?) wird die folgende Teilung der Ebene als Axiom vorausgesetzt
% und dann erhält man das Axiom von Pasch als Satz!!!
...
% ACHTUNG:
% Geht dieser Fall nicht viel einfacher zu klären unter Anwendung des
% folgenden Zusammenhangs:  (den man dann vielleicht auch noch
% beweisen sollte!!!??? Übung!):
...
%S_2$, $\stackrel{Beh.}{\Longrightarrow}\;B\stackrel{g}{\sim} A$,
%d.h. $B\in S_1$.
...
%%% ACHTUNG: 
%% Es fehlt eine Diskussion über die Unabhängigkeit der Anordnungsaxiom!!
%% (angeblich einfach nach Hilbert, aber selber hat er in der
%% 1. Auflage auch einen Fehler gemacht!!!)
...
\begin{proof}
Sei $E\notin g$ und $h=g(A,E)$. Dann teilt $h$ gem\"{a}{\ss}
Theorem \ref{thm:satz.s1a} die Ebene in 2 disjunkte Teilmengen $S_1'$ und
$S_2'$. Die Mengen $S_1=S_1'\cap g$ und $S_2=S_2'\cap g$ erf\"{u}llen
die Bedingungen des Theorems.
...
Mit der Aufteilung einer Ebene bzw. einer Geraden in 2 disjunkte
Teilmengen (zuz\"{u}glich einer Geraden bzw. eines Punktes) haben wir
die Begriffe \glqq{}Strahl\grqq{}, \glqq{}Winkel\grqq{} und \glqq{}Dreieck\grqq{} zur
Verf\"{u}gung.
...
Sei $\E$ eine Ebene, die die Axiome der Inzidenz und der
Anordnung erfüllt.
Seien $A,\,B,\,C\in\E$, paarweise verschieden und nicht kollinear.
\emph{\begin{enumerate}
    \item[\textbf{a)}] \emph{Strahl}
    $\pf{AB}:=\{P\in g(A,B):\;P=A$ oder $P\neq A$ und auf derselben Seite von $A$ wie $B$\}
    \item[\textbf{b)}] \emph{Winkel} $\wi(BAC):=\pf{AB}\cup\pf{AC}$
    \item[\textbf{c)}] \emph{Inneres} von $\wi(BAC):=\{D\in\E: D\stackrel{g(A,B)}{\sim} C$
     und $D\stackrel{g(A,C)}{\sim} B\}$\\
     (Winkel, die kein Inneres besitzen, hei{\ss}en \emph{entartet}.)
     \item[\textbf{d)}] \emph{Dreieck} $ABC$ :=
       $\str{AB}\cup\str{AC}\cup\str{BC}$;\\  $A,B,C$ heißen Ecken,
       $\str{AB},\str{AC},\str{BC}$ heißen Kanten (Seiten) des Dreiecks
    \item[\textbf{e)}] \emph{Inneres} des Dreiecks 
    $ABC :=\\
     \{ D\in\E : D$ geh\"{o}rt zum Inneren von
     $\wi(BAC),\;\wi(ABC),\;\wi(ACB)\}$
%% BEMERUNG: Bei der letzten Definition genügen auch zwei Winkel; 
%% Alternativ kann man auch analog zum Inneren eines Winkels
%% definieren über die drei Beziehungen $\{D\in\E:
%% D\stackrel{g(A,B)}{\sim} C, D\stackrel{g(A,C)}{\sim} B,
%% D\stackrel{g(B,C)}{\sim} A \}$
\end{enumerate}}
\end{defi}


% WIRD DAS FOLGENDE WIRKLICH BEÖTIGT?!?!?
%Wenn kein Missverst\"{a}ndnis m\"{o}glich ist, schreiben wir auch $\wi
%A,\;\wi B,\;\wi C$.
...
Man betrachte beispielsweise \glqq Modell 2\grqq\ (weiter oben), % LINK?
...
\begin{defi}
Ein K\"{o}rper $K$ heißt {\em geordnet},
wenn er eine Teilmenge $K_+\subset K$
(von {\em positiven Elementen}) besitzt, so dass gilt:
\emph{\begin{enumerate}
    \item[(i)] $\forall\,a,\,b\in K_+$ ist $a+b\in K_+$ und $a\cdot
        b\in K_+$;
    \item[(ii)] $\forall\,a\in K$ gilt genau eine der
        Beziehungen $a\in K_+$, $a=0$ oder  $-a\in K_+$.
\end{enumerate}}
\end{defi}
...
auf der $x$-Achse $g$
%der Addition von Strecken
%% ACHTUNG:  HIER MUSS DEFINITITV NOCHMAL ÜBERDACHT WERDEN!!!!!
...
Die Punkte $A$ und $A'$ stehen in einer ganz besonderen Beziehung
...
Nach dem {\em Sehnen-Tangenten-Satz} %(Übung, Blatt 3!!) 
gilt
\[ |MA|\cdot |MA'|=|MP|^2=r^2.\]
...
{\bf (I2), (I3)} sind sicher erfüllt, genau so wie {\bf (A1) - (A3)}. %(\"{U}bung)??
...
%(Kongruenz ist in einem erweiterten Sinn eine \glqq{}Gleichheit\grqq{}):
...
Die Definition der Kongruenz von Strecken erlaubt uns auch mit
Strecken zu rechnen.


\begin{defi}[Addition von Strecken]
\label{def:d1a}
...
und $Zw(EZY)$ folgt   %% aus Lemma \ref{lemma.l1d} 
...
Für einen geordneten Körper~$\K$ gilt $\dist^2(A,B)>0$ genau dann wenn
...
%d.h., wenn wir die Punkte mit denen wir beginnen aus $\K^2$ wählen,
%bleiben wir auch in $\K^2$... 
...
{\bf An dieser Stelle könnte man noch als zweites Beispiel die
...
%    (Diese Aussage ist aus dem Rest beweisbar, so ist es aber
%    bequemer!)\\    
%
...
%    die Dreiecke sind \glqq{}kongruent\grqq{})
...
Sei $\wi(BAC)$ ein Winkel und \pf{AD} ein Strahl im Innern, dann
...
\begin{thm}\label{thm:satz.s1l}  Mit den Bezeichnungen aus der Skizze
  gilt:
...
%% BEMERKUNG: Folgendes sind bei Euklid noch implizite Annahmen oder
%% Axiome (CHECK!!???!!)
...
Die Addition von Winkeln erlaubt auch einen
...
%\subsection*{Kongruenz von Strecken und Winkeln in der Poincar\'eschen
%Kreisscheibe?}
